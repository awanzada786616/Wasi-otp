...
export default async function handler(req, res) {
    ...
    // --- DEBUGGING LOG (Zaroori) ---
    // Mobile se jo bhi aata hai usay Firestore mein save karen check karne ke liye
    ...
    // --- 1. USER SIDE VERIFICATION ACTION ---
    if (action === "verify_user_trx") {
        try {
            const payRef = db.collection("received_payments").doc(trx_id);
            const payDoc = await payRef.get();

            if (!payDoc.exists) return res.status(200).json({ status: "NOT_FOUND", msg: "ID verify nahi hui. Thora wait karen." });

            const payData = payDoc.data();
            if (payData.status === "used") return res.status(200).json({ status: "USED", msg: "Yeh ID pehle hi use ho chuki hai!" });

            // Amount Match Check
            if (Math.abs(parseFloat(payData.amount) - parseFloat(amount)) > 1) {
                return res.status(200).json({ status: "MISMATCH", msg: "Amount match nahi kar rahi!" });
            }
    ...
    // --- 2. MOBILE APP SIDE ACTION (Fixed Regex) ---
    if (secret === "WASI_SECRET_786" && text) {
        // Mazeed flexible regex
    ...
    return res.status(200).json({ success: false, msg: "Data not matched" });
        }
