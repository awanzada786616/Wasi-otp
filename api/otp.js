...
export default async function handler(req, res) {
    ...
    try {
        if (action === 'getNumber') {
            ...
            // 1. Commission wali price jo frontend se aayi (e.g., Rs 4)
            const deductionAmount = Math.ceil(parseFloat(cost));
            if (isNaN(deductionAmount)) return res.status(400).send("ERR_INVALID_PRICE");

            // 2. Supabase se balance check karein
            ...
            // 3. Provider Call (Yahan hum 'cost' NAHI bhej rahe, taake error na aaye)
